...
def may_20 (tup):
  cad_sup = ""
  for x in tup: cad_sup = compara_20(cad_20, x)
  print(cad_sup)
def compara_20(cadenaTMP, cadena):
  if cadena > 20: cadenaTMP = concatena_20(cadenaTMP, cadena)
  return cadenaTMP
def concatena_20(cadenaTMP, cadena):
  if cadenaTMP == "": cadenaTMP = str(cadena)
  else: cadenaTMP = cadenaTMP + ", " + str(cadena)
  return cadenaTMP
...
def word_filter(list_of_words, n):
  x = 1
  for i in range(len(list_of_words)): list_of_words, x = word_list(list_of_words, n, x, i)
...
def contar_vocales(cadena):
  count = 0
  for i in cadena.strip(): count = separa_vocal(i) + count
  return count
def separa_vocal(i):
  if is_vocal(i): return 1
  return 0
...
def square(list):
  ls = []
  for i in range(len(list)): ls.append(list[i]*list[i])
  print(ls)  
...
def rima(word1, word2):
  ...
  if x == "rima un poco": x = rima_comparation2(word1, word2, 3)
  print(x)
